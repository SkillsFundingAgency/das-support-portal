...
namespace SFA.DAS.Support.Portal.UnitTests.Web.Helpers
{
    ...
    public sealed class AccountHelperTests
    {
        ...
        public void ShouldReturnLevyDeclarationDescription(string expected, string dateCeased, bool noPaymentForPeriod,
            string inactiveFrom, string inactiveTo)
        {
            var levyDeclaration = new Declaration
            {
                DateCeased = !string.IsNullOrEmpty(dateCeased)
                    ? DateTime.ParseExact(dateCeased, "dd/MM/yyyy",
                        CultureInfo.InvariantCulture)
                    : (DateTime?) null,
                InactiveFrom = !string.IsNullOrEmpty(inactiveFrom)
                    ? DateTime.ParseExact(inactiveFrom, "dd/MM/yyyy",
                        CultureInfo.InvariantCulture)
                    : (DateTime?) null,
                InactiveTo = !string.IsNullOrEmpty(inactiveTo)
                    ? DateTime.ParseExact(inactiveTo, "dd/MM/yyyy",
                        CultureInfo.InvariantCulture)
                    : (DateTime?) null,
        ...
        public void ShouldReturnYearToDateAmount(string expected, decimal ytdAmount, string dateCeased, bool noPayment,
            string inactiveFrom)
        {
        ...
        public void ShouldReturnSubmissionDate(string expected, int day, int month, int year, int hour, int min,
            int sec)
        {
        ...
        {
            var invitationStatus = (InvitationStatus) Enum.ToObject(typeof(InvitationStatus), status);
