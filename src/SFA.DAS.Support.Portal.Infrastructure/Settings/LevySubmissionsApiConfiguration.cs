...
namespace SFA.DAS.Support.Portal.Infrastructure.Settings
{
    ...
    public class LevySubmissionsApiConfiguration : ILevySubmissionsApiConfiguration
    {
        ...
        public X509Certificate TokenCertificate
        {
            get
            {
                if (string.IsNullOrWhiteSpace(LevyTokenCertificatethumprint)) return null;
                var store = new X509Store(StoreLocation.LocalMachine);
                store.Open(OpenFlags.ReadOnly);
                try
                {
                    var certificates = store.Certificates.Find(X509FindType.FindByThumbprint,
                        LevyTokenCertificatethumprint, false);
                ...
                throw new KeyNotFoundException(
                    $"Couldn't find the certificate for thumbprint '{LevyTokenCertificatethumprint}'");
            }
            set { }
        }

        [JsonRequired] public string LevyTokenCertificatethumprint { get; set; }
    }
}